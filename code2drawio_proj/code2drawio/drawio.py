...
def make_dom_tree(root_dom: SectionDOM)->Tuple[List[str], List[int], List[List[int]]]:
    ...
    children_dom_instance_ids: List[List[int]] = []  # What the DOM Instance Ids are pointed to
    ...
    while len(dom_stack):
        ...
        if isinstance(dom, DocumentDOM):
            ...
            children_dom_instance_ids.append(list(map(child_dom_id, dom.children)))
        ...
        if isinstance(dom, TextGroupDOM):
            ...
            children_dom_instance_ids += \
                [[id(dom.text_lines[i+1])] for i in range(len(dom.text_lines)-1)]            
            children_dom_instance_ids += [[]]   # The last node points to no child node
            continue
        raise ValueError(f'Stack contained something other than DocumentDOM, SectionDOM, SubSecDOM or TextGroupDOM: {type(dom)}')
...
def find_max_width(dom_instance_ids: List[int], dom_instance_id2children_dom_instance_ids: Dict[int, List[int]])->Dict[int, int]:
    ...
    dom_instance_id2children_dom_instance_ids = copy.deepcopy(dom_instance_id2children_dom_instance_ids) # Copy it so that it doesn't affect the input instance
    dom_instance_id2children_max_width: Dict[int, List[int]] = dict([(dom_instance_id, []) for dom_instance_id in dom_instance_ids])
    ...
    while len(parent_dom_stack)>0:
        ...
        if len(dom_instance_id2children_dom_instance_ids[current_dom]):    # DOM still has children DOM to add to queue
            ...
            while len(dom_instance_id2children_dom_instance_ids[current_dom]) != 0:
                ...
                current_dom_stack.append(dom_instance_id2children_dom_instance_ids[current_dom].pop())
            continue
        ...
        dom_instance_id2children_max_width[parent_dom].append(sum(dom_instance_id2children_max_width[current_dom]))
    ...
    for dom_instance_id, children_max_width in dom_instance_id2children_max_width.items():
        dom_instance_id2max_decendent_width[dom_instance_id] = sum(children_max_width)
...
def draw_diagram(section_dom: SectionDOM, diagram: drawio_diagram)->drawio_diagram:
    labels, dom_instance_ids, children_dom_instance_ids = make_dom_tree(section_dom)
    # Calculate the Position of each DOM
    dom_instance_ids2children_dom_instance_ids: Dict[int, List[int]] = dict(zip(dom_instance_ids, children_dom_instance_ids))
    dom_instance_id2max_decendent_width: Dict[int, int] = find_max_width(dom_instance_ids, dom_instance_ids2children_dom_instance_ids)
    ...
    # Draw DOM
    drawio_id_generator = DrawioIdGenerator()
    drawio_ids, diagram = draw_nodes(labels, dom_positions, diagram, drawio_id_generator)
    dom_instance_ids2drawio_ids: Dict[int, str] = dict(zip(dom_instance_ids, drawio_ids))
